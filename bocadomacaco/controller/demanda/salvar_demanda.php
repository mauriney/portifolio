...
try {
    ...
    if ($error == false) {
        ...
        if (isset($_POST['id']) && $_POST['id'] != '') {
            ...
            $stmt = $db->prepare("UPDATE x_demanda SET solicitante = ?, cidade_id = ?, celular = ?, email = ?, prioridade = ?, prazo = ?, demanda = ?, segmento = ?, IdUsuario = ?, quemindicou = ?, data_cadastro = NOW() WHERE id = ?");
            ...
            $stmt->bindValue(10, $quemindicou);
            $stmt->bindValue(11, $_POST['id']);
            ...
            $stmt = $db->prepare("INSERT INTO x_demanda (solicitante, cidade_id, celular, email, prioridade, prazo, demanda, segmento, IdUsuario, quemindicou, status, data_cadastro, confirmacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 2, NOW(), 0)");
            ...
            /* if (is_numeric($demanda_id)) {
              ...
              $stmt2 = $db->prepare("INSERT INTO x_demanda_responsavel (responsavel_id, demanda_id, data_cadastro) VALUES (?, ?, NOW())");
              $stmt2->bindValue(1, $value);
              $stmt2->bindValue(2, $demanda_id);
              $stmt2->execute();

              //ENVIO DE EMAIL DA DEMANDA
              if (is_numeric(pesquisar("IdUsuario", "tb_bsc_usuario", $value, "=", "IdUsuario", "AND IdAcesso <> 1"))) {//NÃO ENVIAR PARA OS ADMINISTRADORES, POIS EMBAIXO JÁ VAI SER ENVIADOS PARA TODOS
              envia_email(pesquisar("Email", "tb_bsc_usuario", $value, "=", "IdUsuario", ""), "Demanda", $email_criado, "Demanda");
              }
              }
            ...
            $msg['id'] = $demanda_id;
            $msg['msg'] = 'success';
            $msg['retorno'] = 'Demanda cadastrada com sucesso.';
            echo json_encode($msg);
            exit();
        }
    } else {
