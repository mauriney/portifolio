...
if (retornaUrl(caminho) == "") {
    $("#titulo1").html("Cadastro de Segmento");
} else {
    $("#titulo1").html("Alteração de Segmento");
}
...
$(document).ready(function () {
    ...
    $('#form_segmento').submit(function () {
        ...
        if (form_valido) {
            projetouniversal.util.getjson({
                url: PORTAL_URL + "controller/configuracoes/segmento/salvar_segmento.php",
                type: "POST",
                data: $('#form_segmento').serialize(),
    ...
    function onSuccessSend(obj) {
        if (obj.msg == 'success') {
            postToURL(PORTAL_URL + 'segmento-painel.php', {mensagem: obj.retorno});
