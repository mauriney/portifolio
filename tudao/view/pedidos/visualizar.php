...
if (is_numeric($id)) {//VERIFICA SE O ID PASSADO POR PARÂMETRO É UM NÚMERO
  $sql = $db->prepare("SELECT p.status, p.funcionario_id, fp.id AS forma_pagamento_id, fp.nome AS forma_pagamento, p.valor_pagamento as pago, p.valor_troco as troco, c.codigo AS cupom, p.id, p.mesa, b.nome AS bairro, p.endereco, p.complemento, p.numero, p.contato
                             FROM pedidos p
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="box box-danger">
                <form id="form_atendimento" name="form_atendimento" action="#" method="post">
                    ...
                    <div class="box-body">
                        ...
                        <br/>
                        <div <?= $reg['status'] == 2 && $reg['mesa'] == NULL ? '' : 'style="display:none"' ?> class="row cart-total">
                            <div class="col-md-6">
                                <label for="funcionario">Entregador</label>
                                <div id="div_funcionario">
                                    <select id="funcionario" name="funcionario" placeholder="Permissões" class="ls-select">
                                        <option value="">Escolha o entregador</option>
                                        <?php
                                        $sql = $db->prepare("SELECT f.id, p.nome
                                FROM funcionarios f
                                LEFT JOIN pessoas AS p ON p.id = f.pessoa_id
                                WHERE f.status = 1 AND p.status = 1
                                ORDER BY p.nome ASC");
                                        $sql->bindValue(1, $id);
                                        $sql->execute();
                                        while ($linha = $sql->fetch(PDO::FETCH_ASSOC)) {
                                          if ($reg['funcionario_id'] == $linha['id']) {
                                            ?>
                        ...
                        <br/>
                        <div <?= $reg['status'] == 2 ? '' : 'style="display:none"' ?> class="box-footer text-center">
...
<!-- JS DO REGISTER -->
<script type="text/javascript" src="<?= PORTAL_URL; ?>scripts/pedidos/visualizar.js"></script>