        ...
        <section class="content">
            ...
            <div class="box box-danger">
                <div class="box-header">
                    <h3 class="box-title">Lista de Bairros</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                    <table id="example1" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Nome</th>
                                <th>Frete em Valor</th>
                                <th>Frete em Pontos</th>
                                <th>Status</th>
                                <th>Ação</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $sql = $db->prepare("SELECT id, nome, valor, pontos, status
                             FROM bairros
                             ORDER BY nome ASC");
                            $sql->execute();
                            while ($linha = $sql->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo "R$ ".fdec($linha['valor']); ?></td>
                                    <td><?php echo $linha['pontos']; ?></td>
                                    <td align="center">
                                        <?php
                                        if ($linha['status'] == 1) {
                                            ?>
                                            <a title="Desativar Bairro" href="#" class="label label-success" onclick="remover(<?php echo $linha['id'] ?>)">
                                                <i class="fa fa-unlock"></i>
                                            </a>
                                            <?php
                                        } else {
                                            ?>
                                            <a title="Ativar Bairro" href="#" class="label label-danger" onclick="ativar(<?php echo $linha['id'] ?>)">
...
<!-- JS DA LISTA -->
<script type="text/javascript" src="<?= PORTAL_URL; ?>scripts/bairros/index.js"></script>
