...
if ($_GET['valor'] != "" && strlen($_GET['valor']) > 0) {//CASO A PESQUISA TENHA ALGUM VALOR INFORMADO IRÁ ENTRAR NO IF ABAIXO
  $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome
            FROM hab_origem_demanda a, seg_usuario u
...
while ($origem_demanda = $result->fetch(PDO::FETCH_ASSOC)) {
  ...
  echo '<tr data-row-id="' . $origem_demanda['id'] . '">
                <td>' . $cont . '</td>
                ...
                <td>
                <a href="' . PORTAL_URL . 'sistema/origem_demanda/cadastro/' . $origem_demanda['id'] . '"><button type="button" class="btn btn-icon palette-Cyan bg waves-effect waves-circle" data-row-id=><span class="zmdi zmdi-edit"></span></button></a>
                <button onclick="remover_acao(' . $origem_demanda['id'] . ')" type="button" class="btn btn-icon btn-danger bg waves-effect waves-circle" data-row-id="' . $origem_demanda['id'] . '"><span class="zmdi zmdi-delete"></span></button>
                </td>
          </tr>';
