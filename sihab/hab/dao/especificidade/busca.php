<?php
//------------------------------------------------------------------------------
//DATA: 17/10/2016 às 15:27
//NOME: Pesquisa Dinâmica
//DESCRIÇÃO: Busca dinâmica da pesquisa do subprograma
//------------------------------------------------------------------------------
@session_start();
...
if ($_GET['valor'] != "" && strlen($_GET['valor']) > 0) {//CASO A PESQUISA TENHA ALGUM VALOR INFORMADO IRÁ ENTRAR NO IF ABAIXO
  $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome 
            FROM hab_especificidade a, seg_usuario u
...
echo '<table id="data-table-basic" class="table table-striped">
          <thead>
            <tr>
              ...
              <th data-column-id="nome">NOME DO PROGRAMA</th>
...
while ($especificidade = $result->fetch(PDO::FETCH_ASSOC)) {
  ...
  echo '<tr data-row-id="' . $especificidade['id'] . '">
                <td>' . $cont . '</td>
                ...
                <td>
                <a href="' . PORTAL_URL . 'sistema/especificidade/cadastro/' . $especificidade['id'] . '"><button type="button" class="btn btn-icon palette-Cyan bg waves-effect waves-circle" data-row-id=><span class="zmdi zmdi-edit"></span></button></a>
                <button onclick="remover_especificidade(' . $especificidade['id'] . ')" type="button" class="btn btn-icon btn-danger bg waves-effect waves-circle" data-row-id="' . $especificidade['id'] . '"><span class="zmdi zmdi-delete"></span></button>
                </td>
          </tr>';
