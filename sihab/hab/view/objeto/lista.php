...
$rs = $db->prepare("SELECT id FROM seg_objeto");
$rs->execute();
...
<div class="card icons-demo">
  <div class="card-header cw-header palette-Light-Blue-700 bg">
    <div class="cwh-year">Objeto</div>
    <div class="cwh-day">Lista</div>
    <?php
      if (vf_objeto_acao("cadastrar") || (vf_objeto_acao("editar"))) {
    ?>
    <a href="<?= PORTAL_URL; ?>sistema/objeto/cadastro" class="btn palette-Light-Green bg btn-float waves-effect waves-circle waves-float"><i class="zmdi zmdi-plus"></i></a>
    <?php
    }
    ?>
  </div>
          ...
          <tbody>
            ...
            if ($busca == "") {
              $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome 
                                 FROM seg_objeto a, seg_usuario u WHERE a.usuario_pai_id = u.id ORDER BY upper(a.nome) ASC");
            } else {
              $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome 
                                 FROM seg_objeto a, seg_usuario u WHERE a.usuario_pai_id = u.id and a.nome like ? ORDER BY upper(a.nome) ASC");
...
<!-- JS DO OBJETO-LISTA -->
<script type="text/javascript" src="<?= PORTAL_URL; ?>hab/js/objeto/lista.js"></script>