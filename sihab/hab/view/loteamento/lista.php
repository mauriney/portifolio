...
$rs = $db->prepare("SELECT id from snch_loteamento");
$rs->execute();
...
<div class="card icons-demo">
  <div class="card-header cw-header palette-Pink-700 bg">
    ...
    ?>
    <a href="<?= PORTAL_URL; ?>sistema/loteamento/cadastro" class="btn palette-Light-Green bg btn-float waves-effect waves-circle waves-float"><i class="zmdi zmdi-plus"></i></a>
    <?php
    }
    ?>
  </div>
          ...
          <tbody>
            ...
            if ($busca == "") {
              $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome 
                                 FROM snch_loteamento a, seg_usuario u WHERE a.seg_usuario_pai_id = u.id ORDER BY upper(a.nome) ASC");
            } else {
              $result = $db->prepare("SELECT a.nome, a.data_cadastro, a.id, a.status, u.nome as usuario_pai_nome 
                                 FROM snch_loteamento a, seg_usuario u WHERE a.seg_usuario_pai_id = u.id and a.nome like ? ORDER BY upper(a.nome) ASC");
...
<!-- JS DO OBJETO-LISTA -->
<script type="text/javascript" src="<?= PORTAL_URL; ?>hab/js/loteamento/lista.js"></script>