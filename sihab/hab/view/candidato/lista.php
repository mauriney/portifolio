...
<?php
$busca = (isset($_POST['buscar_form']) ? $_POST['buscar_form'] : '');
...
$opVlrMin = isset($_REQUEST['renda_valor_min']) && $_REQUEST['renda_valor_min'] != '' ? (array(
    ...
    'query' => $_REQUEST['renda_valor_min']
        )) : (array(
    'value' => NULL,
    'query' => ''
        ));
$opVlrMax = isset($_REQUEST['renda_valor_max']) && $_REQUEST['renda_valor_max'] != '' ? (array(
    ...
    'query' => $_REQUEST['renda_valor_max']
        )) : (array(
    'value' => NULL,
    'query' => ''
        ));
...
$stmt = $db->prepare("SELECT hc.seg_usuario_pai_id, hc.id, hp.id AS pessoa_id, hp.nome, hp.cpf, hp.email, hp.cadastro_unico, hp.data_nascimento, bm.nome AS municipio, hc.situacao, 
                      ...
                      (SELECT SUM(hpe.valor) AS vlr FROM hab_pessoa_renda AS hpe WHERE hpe.hab_pessoa_id = hp.id AND hpe.hab_renda_tipo_id = 1) AS vlr_renda_comp, 
                      (SELECT SUM(hpe.valor) AS vlr FROM hab_pessoa_renda AS hpe WHERE hpe.hab_pessoa_id = hp.id AND hpe.hab_renda_tipo_id = 2) AS vlr_renda_ncomp 
                      ...
                      WHERE 1 = 1 AND hp.cadastro_retroativo_ano IS NULL AND hp.nome like ?
                      ...
                      GROUP BY hc.id, hp.id, hp.nome, hp.cpf, hp.email, hp.cadastro_unico, hp.data_nascimento, bm.nome, hc.situacao, qtd_filho, vlr_renda_comp, vlr_renda_ncomp 
                      ORDER BY hc.id ASC");
...
<form id="form_lista" action="<?= PORTAL_URL ?>sistema/candidato/lista" method="post">
    ...
    <div class="card-body card-padding">
      ...
      <div role="tabpanel">
        <ul class="tab-nav" role="tablist">
          <li class="active"><a href="#aprovados" class="aprovados" aria-controls="aprovados" role="tab" data-toggle="tab">APROVADOS</a></li>
          <li><a href="#cadastrando" class="cadastrando" aria-controls="cadastrando" role="tab" data-toggle="tab">CADASTRANDO</a></li>
          <li><a href="#aguardando" class="aguardando" aria-controls="aguardando" role="tab" data-toggle="tab">AGUARDANDO VALIDAÇÃO</a></li>
          <li><a href="#retornados" class="retornados" aria-controls="retornados" role="tab" data-toggle="tab">DEVOLVIDO PARA CORREÇÃO</a></li>
        </ul>
        <div class="tab-content">
          <div role="tabpanel" class="tab-pane" id="cadastrando">
...
<!--                     <th data-column-id="cadunico" data-formatter="cadunico">CAD. ÚNICO</th> -->
                ...
                <tbody>
                  ...
                  foreach ($rsCandidatos as $kCandidato => $objCandidato) {
                    if ($objCandidato['situacao'] == 1) {
                      if (isset($objCandidato['id'])) {
                        if ($opQtdFilho['value'] == - 1 || $objCandidato['qtd_filho'] == $opQtdFilho['value']) {
                          if ($somaRendaTipo == 0 || $somaRendaTipo == 7 || (($objCandidato['vlr_renda_comp'] > 0 && in_array($somaRendaTipo, $vetorRendaComp)) || ($objCandidato['vlr_renda_ncomp'] > 0 && in_array($somaRendaTipo, $vetorRendaNComp)) || (($objCandidato['vlr_renda_comp'] == 0 && $objCandidato['vlr_renda_ncomp'] == 0) && in_array($somaRendaTipo, $vetorRendaInex)))) {
                            //if ($objCandidato['seg_usuario_pai_id'] == $_SESSION['id']) {
                            ?>
                            <tr data-row-id="<?= $objCandidato['id']; ?>">
                              <td><?= $objCandidato['id']; ?></td>
                              <td><?= $objCandidato['nome']; ?></td>
                              <td><?= $objCandidato['cpf']; ?></td>
          <!--                                 <td><?//= $objCandidato['municipio']; ?></td> -->
          <!--                                 <td><?//= $objCandidato['celular']; ?></td> -->
          <!--                                 <td><?//= $objCandidato['cadastro_unico']; ?></td> -->
                            </tr>
                            <?php
                            //}
                          }
          ...
          <div role="tabpanel" class="tab-pane" id="retornados">
            <div id="pagina" class="table-responsive">
              <table id="data_table_candidato" class="table table-striped table-candidate">
                <thead>
                  <tr>
...
<!--                     <th data-column-id="cadunico" data-formatter="cadunico">CAD. ÚNICO</th> -->
          ...
          <div role="tabpanel" class="tab-pane" id="aguardando">
            <div id="pagina" class="table-responsive">
              <table id="data_table_candidato" class="table table-striped table-candidate">
                <thead>
                  <tr>
...
<!--                     <th data-column-id="cadunico" data-formatter="cadunico">CAD. ÚNICO</th> -->
                ...
                <tbody>
                  ...
                  foreach ($rsCandidatos as $kCandidato => $objCandidato) {
                    if ($objCandidato['situacao'] == 2) {
                      if (isset($objCandidato['id'])) {
                        if ($opQtdFilho['value'] == - 1 || $objCandidato['qtd_filho'] == $opQtdFilho['value']) {
                          if ($somaRendaTipo == 0 || $somaRendaTipo == 7 || (($objCandidato['vlr_renda_comp'] > 0 && in_array($somaRendaTipo, $vetorRendaComp)) || ($objCandidato['vlr_renda_ncomp'] > 0 && in_array($somaRendaTipo, $vetorRendaNComp)) || (($objCandidato['vlr_renda_comp'] == 0 && $objCandidato['vlr_renda_ncomp'] == 0) && in_array($somaRendaTipo, $vetorRendaInex)))) {
                            if (vf_objeto_acao("validador")) {
                              ?>
                              <tr data-row-id="<?= $objCandidato['id']; ?>">
                                <td><?= $objCandidato['id']; ?></td>
                                <td><?= $objCandidato['nome']; ?></td>
          ...
          <div role="tabpanel" class="tab-pane active" id="aprovados">
            <div id="pagina" class="table-responsive">
...
<!-- JS DO OBJETO-LISTA -->
<script type="text/javascript" src="<?= PORTAL_URL; ?>hab/js/candidato/lista.js"></script>
<script>
